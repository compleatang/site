...
1. Save the favicon SVG to `static/favicon.svg`
2. If the user wants a traditional `favicon.ico`, suggest using an online converter or ImageMagick:
   ...
   ```
3. Place `favicon.ico` in `static/` (seite copies `static/` contents to `dist/` root)
...
| `static/favicon.svg` | Favicon (simplified SVG) |
| `static/favicon.ico` | Favicon (traditional, if converted) |
